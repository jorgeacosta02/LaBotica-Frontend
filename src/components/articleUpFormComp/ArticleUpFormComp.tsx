...
//               />
//               {errors.unit && (
//                 <p className={styles.errorMessage}>{errors.unit}</p>
//               )}
//             </div>
//             <div className={styles.inputBlock}>
...
interface ArticleErrors {
  ...
  unit?: string;
  price?: string;
  image?: string; // La imagen es opcional y puede ser una cadena de error
}
...
const ArticleUpFormComp = () => {
  ...
  const submitForm = async () => {
    try {
      if (!formInfo.image) {
        // Si no hay imagen, actualizamos el estado de errores
        setErrors((prevErrors) => ({
          ...prevErrors,
      ...
      const formData = new FormData();
      formData.append('file', formInfo.image);
      formData.append('upload_preset', 'Presets_react');
      ...
      const articleResponse = await axios.post(
        'http://localhost:4000/article',
        { ...formInfo, image: imageUrl }
      );

      console.log('articleResponse', articleResponse.status);
          ...
          <div className={styles.dataContainer}>
            ...
            <div className={styles.inputBlock}>
              ...
              <div className={styles.inputBlock}>
                ...
                <ImageUploading
                  value={formInfo.image ? [{ dataURL: formInfo.image }] : []}
                  onChange={handleImageChange}
                  maxNumber={1}
                  dataURLKey="dataURL"
                >
